...
colors = [(217, 67, 67), (217, 157, 67), (130, 217, 67), (67, 217, 109), (67, 217, 197), (67, 145, 217), (215, 67, 217)]
algorithm_stack = []
all_past_algorithms = []
...
class MainMenu(ColorConstants):
    def __init__(self, SCREEN_WIDTH, SCREEN_HEIGHT):
        ...
        for i in range(3,6):
            ...
            btn.rect.x = (self.SCREEN_WIDTH - (5 * (btn.rect.width + self.spacing) - self.spacing)) / 2 + (i - 3) * (btn.rect.width + self.spacing) + 30
        ...
        self.end_position_button = Button("Izberite kot končno stanje", self.BLACK, 30, None, self.BUTTON_COLOR, 300, 30)
    ...
    def choose_gamemode(self):
        ...
        button_texts = ["Preiskovanje v globino", "Preiskovanje z iterativnim poglabljanjem"]
        button_height = 30
        spacing = 20
        ...
        for text in button_texts:
            ...
            btn = Button(text, self.BLACK, button_height, None, self.BACKGROUND, btn_width, btn_height)
            btn.rect.x = start_x
            btn.rect.y = self.SCREEN_HEIGHT / 2 - 50
            btn.render_text()
...
class MoveAction():
    def __init__(self, element, start, end):
        ...
        self.preconditions = [State("clear", element), State("on", element, start), State("clear", end)]
        self.add_effects = [State("on", element, end), State("clear", start)]
        self.delete_effect = [State("on", element, start), State("clear", end)]
        self.limitations = [State("block", element), State("not_equal", start, end), State("not_equal", start, element), State("not_equal", end, element)]
...
class Algorithm(ColorConstants):
    def __init__(self, SCREEN_WIDTH, SCREEN_HEIGHT, game):
        ...
        self.next_button = Button(">", self.BLACK, 30, None, self.GREEN, self.BUTTON_WIDTH + 10 * 2, self.BUTTON_HEIGHT + 10 * 2)
        ...
        self.retry_button = Button("Retry", self.BLACK, 30, None, self.GREEN, self.BUTTON_WIDTH + 10 * 4, self.BUTTON_HEIGHT + 10 * 2)
        ...
        self.plan_cursor = ((self.plan_cursor_x, self.plan_cursor_y), \
    ...
    def draw_algorithm(self, screen):
        ...
        if self.current_action != None and self.current_action.show:
            current_action_text = font.render("Izberemo akcijo: " + self.get_action_text(self.current_action), True, (0,0,0))
        ...
        for i in range(len(self.plan)):
            action = self.plan[i]
            if action != None:
                action_color = self.BLACK if action.completed else (self.GREEN if len([precondition for precondition in action.preconditions if not precondition.is_true(self.game.positions)]) == 0  else self.RED)
        ...
        if self.show_info and self.info_text != "":
            ...
            info_box = Block(self.INFO_COLOR, info_text_rect.width + text_margin * 2, info_text_rect.height + text_margin * 2)
        ...
        # Render plan cursor
        self.plan_cursor_x = 30 + self.SCREEN_WIDTH - self.PLAN_WIDTH - 25
        self.plan_cursor_y = 50 + plan_rect.height * (self.plan_pointer + 1) + text_margin * (self.plan_pointer + 2) + ((1 / 2) * self.font_size - 22)
        self.plan_cursor = ((25,75),(320,125),(250,375))
        self.plan_cursor = ((self.plan_cursor_x, self.plan_cursor_y), \
                            (self.plan_cursor_x, self.plan_cursor_y + 16), \
                            (self.plan_cursor_x + 16, self.plan_cursor_y + 8))
        ...
        for i in range(4):
            text = font.render(["predpogoji: [clear(Block), on(Block,From), clear(To)]", "učinki-add [on(Block,To), clear(From)]", "učinki-del: [on(Block,From), clear(To)]", "omejitve: [block(Block), To≠From, To≠Block, From≠Block]"][i], True, self.BLACK)
        ...
        if self.search_by_width:
            ...
            history_init_text_rect.y = self.PLAN_HEIGHT + 100 + (i * 3) * text_margin + i * pygame.font.Font(None, self.font_size).render("x", True, self.BLACK).get_rect().height - 200 + history_init_text_rect.height
            screen.blit(history_init_text, history_init_text_rect)
                  
            for i in range(len(algorithm_stack)):
                alg_rect = Block(self.ACTION_INFO_COLOR, 100, 10 * 2 + pygame.font.Font(None, self.font_size).render("x", True, self.BLACK).get_rect().height)
                alg_rect.rect.x = 50
                alg_rect.rect.y = self.PLAN_HEIGHT + 100 + (i * 3) * text_margin + i * pygame.font.Font(None, self.font_size).render("x", True, self.BLACK).get_rect().height
        ...
        if self.showAlgorithm >= 0: # goals, selected goal, actions, plan, current_state
            ...
            for i in range(len(alg.plan)):
                action = alg.plan[i]
                if action != None:
                    action_color = self.BLACK if action.completed else (self.GREEN if len([precondition for precondition in action.preconditions if not precondition.is_true(alg.game.positions)]) == 0  else self.RED)
                    plan_action_text = font.render(alg.get_action_text(alg.plan[i]), True, action_color)
                    plan_action_text_rect = plan_action_text.get_rect()
                    plan_action_text_rect.x = plan_left_margin
                    plan_action_text_rect.y = top_margin + plan_action_text_rect.height * (i + 1) + text_margin * (i + 1)
                    screen.blit(plan_action_text, plan_action_text_rect)
               
    def do_next(self):
        # go through plan and check for loops
        if len(list({(action.element, action.start, action.end): action for action in self.plan}.values())) + 1 < len(self.plan):
        ...
        if self.search_by_width:
            if self.actions_index >= len(self.possible_actions) and len(self.possible_actions) != 0:
                ...
                return
            if len(self.plan) == self.limit and len([action for action in self.plan if action.completed or len([precondition for precondition in action.preconditions if not precondition.is_true(self.game.positions)]) > 0]) == len(self.plan):
                self.limit_reached = True
                self.go_one_step_back = True
                return
            self.limit_reached = False
        ...
        if len(self.plan) > 0 and self.current_action != None and self.current_action == next_action and do_action:
            ...
            next_action = not_completed_actions[0] if len(not_completed_actions) > 0 else None
            if next_action != None:
                preconditions_met = len([x for x in next_action.preconditions if not x.is_true(self.game.positions)]) == 0
            ...
            if self.current_goal == None:
                ...
                if self.current_action == None and (self.possible_actions == None or len(self.possible_actions) == 0):
                    ...
                    if self.search_by_width and len(self.plan) + 1 == self.limit and self.possible_actions != None and len(self.possible_actions) > 0 and self.count_true_preconditions(self.possible_actions[self.actions_index]) != len(self.possible_actions[self.actions_index].preconditions):
                        self.go_one_step_back = True
                        self.actions_index_too_close_to_limit = True
                        return
                    if self.actions_index == len(self.possible_actions): # preveri ali smo na koncu vseh možnih akcij ali pa je tako ali tako brez veze probavat ker smo 1 od limite in nima več vseh predpogojev izpolnjenih
                        self.go_one_step_back = True
                    else:
                        new_algorithm = self.get_algorithm_copy()
                        algorithm_stack.append(new_algorithm)
                        self.choose_action(self.actions_index)
                        self.info_text = "Izberemo akcijo z največ izpolnjenimi predpogoji" if self.actions_index == 0 else "Izberemo naslednjo akcijo po vrsti"
                else:
    ...
    def get_possible_actions(self):
        ...
        for a in all_actions:
            ...
            for e in a.add_effects:
                if e.name == self.current_goal.name and e.a == self.current_goal.a and e.b == self.current_goal.b and \
                    a.element != a.end and a.start != a.end and a.element != a.start:
                    adds_correct_effect = True
                    break
            if adds_correct_effect:
                filtered_actions.append(a)
        self.possible_actions = sorted(filtered_actions, key=lambda action: (-self.count_true_preconditions(action), str(action.element), str(action.start), str(action.end)))
    ...
    def get_states_text(self, states_arr = None):
        ...
        for i in range(len(states_arr)):
            state = states_arr[i]
            current_state_text += state.name + "(" + str(state.a) + ((", " + str(state.b)) if state.b is not None else "") + ")"
    ...
    def get_action_text(self, action, preconditions = False):
        if action == None:
            return ""
        if preconditions:
            return "move(" + str(action.element) + ", " + str(action.start) + ", " + str(action.end) + ") -> " + self.get_states_text(action.preconditions)
        ...
        for position in self.game.positions:
            for i in range(len(position.discs)):
                if alphabet[position.discs[i].id] == e: # Find the correct element to move
                    if (start not in alphabet and position.pos_index == start - 1 and i == 0) or (i != 0 and alphabet[position.discs[i - 1].id] == start): # If the start variable holds up
            ...
            if len(pos.discs) == 0:
                ...
                for i in range(len(pos.discs)):
                    ...
                    if i + 1 == len(pos.discs):
                        ...
                        self.current_state.append(State("on", alphabet[pos.discs[i + 1].id], alphabet[pos.discs[i].id]))
            ...
            if len(pos.discs) == 0:
                ...
                for i in range(len(pos.discs)):
                    if i + 1 == len(pos.discs):
                        ...
                        self.goals[-1].append(State("on", alphabet[pos.discs[i + 1].id], alphabet[pos.discs[i].id]))
...
class Game(ColorConstants):
    def __init__(self, SCREEN_WIDTH, SCREEN_HEIGHT, number_of_discs):
        ...
        self.game_board.rect.center = (SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 + SCREEN_HEIGHT * 0.25)
    ...
    def set_new_positions(self):
        ...
        self.POS_WIDTH = (self.BOARD_WIDTH - (self.number_of_positions - 1) * self.POS_SPACING) / self.number_of_positions
        self.POS_HEIGHT = 400
    ...
    def refresh_discs(self):
        ...
        for i in range(len(self.discs)):
            ...
            disc.rect.y = (self.BOARD_Y - self.DISC_HEIGHT) - self.DISC_HEIGHT * self.positions[disc.current_pos].discs.index(disc)
        self.sprites_list.add(self.discs)
