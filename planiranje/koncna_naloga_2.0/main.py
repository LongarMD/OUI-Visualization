...
while not done:
    if algorithm_running and algorithm.search_by_width and algorithm.go_one_step_back:
        ...
        if len(models.algorithm_stack) > 1:
            ...
            info_text = ""
            if algorithm.actions_index_too_close_to_limit:
                info_text = "Ne splača se več preiskovati akcij, ker nobena nova nima izpolnjenih predpogojev in smo preblizu limite. Zvišamo limito na " + str(algorithm.limit + 1)
            else:
                info_text = "Šli smo čez vse akcije. Limito moramo povečati za 1. Vrnemo se na začetno stanje in preiskujemo od začetka z limito " + str(algorithm.limit + 1)
            ...
            if init_game:
                ...
                if menu.start_position_button.is_clicked():
                    ...
                    for i in range(game.number_of_discs):
                        disc = models.Disc(game.discs[i].current_pos, i, models.colors[i], game.DISC_WIDTH, game.DISC_HEIGHT)
        ...
        if drag:
            ...
            if move:
                ...
                if change:
                    ...
                    game.discs[disc_index].rect.x = game.positions[new_pos].rect.x - (game.DISC_WIDTH / 2) + game.POS_WIDTH / 2
