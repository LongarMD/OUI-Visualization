...
def mep(dtree, attr, pa=dict(), m=1, ca=False, lapl=False):
    ...
    def est(dtree, attr):
        ...
        if lapl:
            if ca:
                ...
                e = 1 - (n + 1) / (N + k)
                return e, "$=1-\\frac{n+1}{N+k}=1-\\frac{%d+1}{%d+%d}=%f$" % (
                    n,
                    N,
                    k,
                    e,
            ...
            if ca:
                ...
                return (
                    e,
                    "$=\\frac{n+p_a(%s)\\cdot m}{N+m}=\\frac{%d+%f\\cdot %f}{%d+%d}=%f$"
                    % (attr, n, pa[attr], m, N, m, e),
                )
            else:
                e = 1 - (n + pa[attr] * m) / (N + m)
                return (
                    e,
                    "$=1-\\frac{n+p_a(%s)\\cdot m}{N+m}=1-\\frac{%d+%f\\cdot %f}{%d+%d}=%f$"
                    % (attr, n, pa[attr], m, N, m, e),
                )
    ...
    def stat_est(st):
        ...
        for k, _ in st["classification#"].items():
            ...
            frm += [
                "p(%s$%s$%s$|$current node) =" % (attr, "=" if ca else "\\neq ", k),
                f,
            ]
            i += 1
...
def render_trace(trace, font_size=13):
    ...
    tbl = tbl.rename(
        lambda x: x + " - class" if x == trace["table"].columns[-1] else x,
        axis="columns",
    )
    tbl.index.name = ""
