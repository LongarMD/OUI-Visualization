...
def visualize(data, labels, ax,showTestPoint=False, test_point=[],k=0, classified_class=None):
    ...
    for class_label, color in zip(np.unique(labels), colors):
        ...
        ax.scatter(class_data[:, 0], class_data[:, 1], c=[color], label=f'Class {class_label}', alpha=0.7, edgecolors='w', s=100)

    if showTestPoint:
        
        if classified_class is not None:
            classified_color = colors[classified_class]
            ax.scatter(test_point[0], test_point[1], c=[classified_color], marker='o', s=200, alpha=0.7)
        else:
        ...
        if k > 0:
            ...
            circle = patches.Circle((test_point[0], test_point[1]), radius, fill=False, color='blue', linestyle='--', linewidth=2)
            ax.add_patch(circle)
...
def on_show_knn():
    ...
    msgbox.showinfo("Test Point Classification", f"The test point belongs to class {test_point_class}")
